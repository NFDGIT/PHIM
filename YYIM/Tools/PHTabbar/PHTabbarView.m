        ...
        if ([btn isKindOfClass:[UIButton class]]) {
            ...
            if (btn.tag-100==_index) {
                ...
                [UIView animateWithDuration:duration animations:^{
                    self->_titleLine.width=btn.width;
                    if (self->_tabbarType==TabbarTypeScrollRound) {
                        [self sendSubviewToBack:self->_titleLine];
                        self->_titleLine.frame=btn.frame;
                        self->_titleLine.backgroundColor=[UIColor redColor];
                        self->_titleLine.layer.cornerRadius=3;
                        self->_titleLine.layer.masksToBounds=YES;
                    }
                    self->_titleLine.centerX=btn.centerX;
                    self->_titleLineWithTem=self->_titleLine.width;
